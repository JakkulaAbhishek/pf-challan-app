...
def generate_pdf_summary(df, total_pf, disallowance, late_count):
    ...
    w = [35, 30, 30, 20, 35, 35, 35, 30, 25] 
    headers = ["Wage Month", "Due Date", "Paid Date", "Late", "Employer Rs", "Employee Rs", "Total Rs", "Disallowance", "Status"]
    ...
    for _, row in df.iterrows():
        ...
        pdf.cell(w[7], 8, f"{row['PF Disallowance (INR)']:,.2f}", 1, 0, 'R')
        pdf.cell(w[8], 8, "LATE" if row['Late Days'] > 0 else "OK", 1, 1, 'C')
...
if files and run_audit:
    ...
    if all_rows:
        ...
        with c1:
            fig = px.bar(df, x='Wage Month', y='Grand Total (INR)', color='Late Days', title="PF Contribution & Delay Trend", color_continuous_scale='Reds')
        ...
        st.dataframe(df.style.format({"Grand Total (INR)": "{:,.2f}", "PF Disallowance (INR)": "{:,.2f}", "Employee Share (INR)": "{:,.2f}"}), use_container_width=True)
    else:
        st.error("❌ NO DATA FOUND: Ensure the PDF is a digital EPFO challan receipt.")
